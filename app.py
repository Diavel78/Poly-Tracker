...
PROPS_CACHE_TTL = 120  # 2 minutes — prop lines don't move much
...
def _normalize_props(raw_props):
    """Parse props into a list of games, each with players and their props.

    Expected API structure (similar to odds): top-level `data` keyed by book,
    each book has events with player prop markets.
    ...
    Returns list of:
    {
        ...
        "players": {
            "Player Name": {
                "props": {
                    "market_key": {   # e.g. "player_strikeouts", "player_points"
                        "label": "Strikeouts",
                        "point": 5.5,
                        "books": {
                            "fanduel": {"over": -120, "under": +100, "point": 5.5, "link": "..."},
                            ...
                        }
                    }
                }
    ...
    # The props endpoint may be structured like odds (keyed by book) or as a flat list.
    # Handle both formats.

    # Format 1: { "data": { "book_key": [ events... ] } } (like odds endpoint)
    if isinstance(raw_data, dict):
        return _normalize_props_by_book(raw_data)

    # Format 2: { "data": [ events with embedded bookmakers ] } (like a flat list)
    if isinstance(raw_data, list):
...
def _normalize_props_by_book(data_by_book):
    ...
    for book_key, events in data_by_book.items():
        ...
        for ev in events:
            ...
            game = games[eid]
            # Extract player prop markets from bookmakers
            for bm in ev.get("bookmakers", []):
                ...
                for mkt in bm.get("markets", []):
                    ...
                    if not mkt_key:
                        continue
                    # Player prop markets typically have "player" in the key
                    # or have a "description" field with the player name
                    description = mkt.get("description", "")
                    outcomes = mkt.get("outcomes", [])
                    if not outcomes:
                        continue

                    # Determine player name from description or outcomes
                    player_name = description
                    if not player_name:
                        # Try to extract from outcome descriptions
                        for oc in outcomes:
                    ...
                    # Build a composite key for this specific prop line
    ...
    result = sorted(games.values(), key=lambda g: g.get("commence_time", ""))
    return result


def _normalize_props_flat(data_list):
    """Normalize props when data is a flat list of events with embedded bookmakers."""
    games = []
    for ev in data_list:
        eid = ev.get("eventId") or ev.get("event_id") or ev.get("id", "")
        game = {
...
def _prop_market_label(key):
    ...
    if key in labels:
        return labels[key]
    # Try removing common suffixes like _over, _alternate
